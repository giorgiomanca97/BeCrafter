...
public class BuyBeerController {
	private static BuyBeerController instance = null;
	...
	public static synchronized BuyBeerController getInstance() {
		if(instance == null) {
			instance = new BuyBeerController();
		}
	...
	public String confirmPurchase(String email, BillingInfo billingInfo) throws EmptyCartException, UsedEmailException, PaymentRefusedException, IdException {
		...
		if(RegisteredDao.getRegisteredByEmail(email) != null && !LoginController.getInstance().isLogged(email)) {
			throw new UsedEmailException("Email already used. Please login!");
		}
		
		String lastOrderId = OrderDao.getLastId();
		String orderId;
