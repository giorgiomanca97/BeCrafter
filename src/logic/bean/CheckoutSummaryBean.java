...
public class CheckoutSummaryBean{
	...
	public void loadSelectedProduct() {
		Product product = BuyBeerController.getInstance().getSelectedProduct();
	...
	public boolean selectProductInCart(int index) {		
		return BuyBeerController.getInstance().selectProductInCart(index);
	}
	
	public int cartSize() {
		return BuyBeerController.getInstance().getCartSize();
	}
	...
	public void updateProductInsideCart() throws ProductNotFoundException, StorableIllegalQuantityException {
		...
		BuyBeerController.getInstance().updateProductInsideCart(csBeerId, csContainerType, volume, csQuantity);
	}
	
	public void removeProductFromCart() throws ProductNotFoundException {
		Volume volume = new Volume(csContainerVolume);
		BuyBeerController.getInstance().removeProductFromCart(csBeerId, csContainerType, volume);
	}
}
