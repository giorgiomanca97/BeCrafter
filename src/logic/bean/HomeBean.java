...
import logic.BuyBeerController;
...
public class HomeBean {
	...
	public void displayProducts(List<BeerType> beerTypes, List<BeerColor> beerColors, List<ContainerType> containerTypes, List<BeerFiltering> beerFilterings, String searchName){
		HomeController.getInstance().displayProducts(beerTypes, beerColors, containerTypes, beerFilterings, searchName);
	}
	
	public int countDisplayedProducts() {
		return HomeController.getInstance().countProductsDisplayed();
	}
	
	public void loadDisplayedProductAt(int index) {
		Product product = HomeController.getInstance().getDisplayedProductsAt(index);
	...
	public void selectForSaleProduct() throws ProductNotFoundException {
		...
		BuyBeerController.getInstance().selectProductForSale(hBeerId, hContainerType, volume);
	}
	
	public String loggedCustomer() {
		Registered registered = LoginController.getInstance().getLoggedCustomer();
	...
	public void logoutCustomer(String email) {
		LoginController.getInstance().logout(email);
	}
}
